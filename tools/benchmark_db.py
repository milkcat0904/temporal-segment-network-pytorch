...
def parse_ucf_splits():
    class_ind = [x.strip().split() for x in open('list/ucf101_splits/classInd.txt')]
    ...
    splits = []
    for i in range(1, 4):
        train_list = [line2rec(x) for x in open('list/ucf101_splits/trainlist{:02d}.txt'.format(i))]
        test_list = [line2rec(x) for x in open('list/ucf101_splits/testlist{:02d}.txt'.format(i))]
...
def parse_hmdb51_splits():
    # load split file
    class_files = glob.glob('list/hmdb51_splits/*split*.txt')

    # load class list
    class_list = [x.strip() for x in open('list/hmdb51_splits/class_list.txt')]
